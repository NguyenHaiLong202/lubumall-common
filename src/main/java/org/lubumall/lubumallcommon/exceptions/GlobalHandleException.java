...
public class GlobalHandleException extends ResponseEntityExceptionHandler {
    ...
    public ResponseEntity<ExceptionResponse> globalExceptionHandler(Exception ex, HttpServletRequest request) {
        ...
        if (ex instanceof ExceptionLb exceptionLb) {
            ExceptionResponse exceptionResponse = new ExceptionResponse(exceptionLb.status, new Date(), exceptionLb.getMessage(), exceptionLb.messageCode, exceptionLb.getMessage(), request.getServletPath());
        ...
        try {
            ...
            if (ex instanceof ExceptionLb exceptionLb) {
                if (exceptionLb.status.value() < 500) {
                    ...
                    msg.append("<b>ERROR</b> : ").append(ex.getMessage()).append(" (").append(exceptionLb.status.value()).append(") \n");
                }
            } else {
